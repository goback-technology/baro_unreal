...
- `Config/` — 5개 파일: `DefaultEditor.ini`, `DefaultEngine.ini`(RHI·맵·게임모드·`[HTTPServer.Listeners]` + **`r.Lumen.HardwareRayTracing=True`** — 캡처 persist 가드와 짝, memo 「메모리 누수 원인과 대응」), `DefaultGame.ini`(플러그인 서브시스템 설정 + `[GeneralProjectSettings] ProjectVersion` = **앱 버전**, 현 **0.2.8**(배포 zip 이름의 출처 — `Scripts/package.ps1 -Zip`) + AssetManager **GameFeatureData 항목 bIsEditorOnly=True 필수** — memo 「반복 금지」), `DefaultGameUserSettings.ini`(배포 기본 960×540 일반 창모드 `FullscreenMode=2` + Epic 품질·`sg.ResolutionQuality=100`), `DefaultInput.ini`(창모드 고정을 위해 `bAltEnterTogglesFullscreen=False` / `bF11TogglesFullscreen=False`).
...
- **실행**: `./Scripts/run.ps1 [-Mode Game|Editor|Packaged] [-Map ...] [-ResX/-ResY] [-ScenePort N] [-Fullscreen] [-Log] [-WaitForExit]`
  (`-ScenePort` = 씬 제어 포트 지정, 다중 인스턴스 분리용 — 플러그인 v0.1.16 커맨드라인 스위치로 전달. Base 포트는 `-ExtraArgs "-BaseHttpPort=..."`.)
...
- **패키지 실행 검증**: `Packaged/Win64/baro_unreal.exe` 실행 → 로그 `[Hucoms] 시뮬레이터 서버 시작 — 채널 N/N` 확인. 실 게임/소켓은 **자식 프로세스**(Shipping은 `baro_unreal-Win64-Shipping.exe`)가 소유하므로 **포트 기준**으로 확인:
  ...
  ```
  제어 = `BaseHttpPort 8081`+카메라인덱스, MJPEG = `BaseMjpegPort 8091`+인덱스, 씬 제어 = 기본 8095(런치 `-ScenePort=N` 으로 인스턴스별 지정 — v0.1.16, `memo.md` 「다중 인스턴스」). 제어 포트·씬 포트는 플러그인이 여는 지점에서 전 인터페이스로 선언하므로 ini 목록이 필요 없다(`memo.md` 「기본 설정값」).
