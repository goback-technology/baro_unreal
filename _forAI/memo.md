...
- 플러그인 버전은 `baroCCTVSimulator.uplugin` `VersionName`이 단일 출처다. 현재 **0.1.7**(0.1.2=차종 카탈로그 리플렉션, 0.1.3=캡처 VT 스로틀 해제, 0.1.4=setcenter tan+구면 기하, 0.1.5=캡처 persist off — 0.1.6 으로 대체, 0.1.6=persist HWRT 가드, 0.1.7=/scene API 확장[차량 boundsCm·카메라 groundReference/heightAboveReferenceGroundCm·intrinsics.zoomHfov] **+ force 스폰 슬롯 축출 수정**)이며 `/scene/catalog.pluginVersion`, 웹 `/simulator` 씬 카드, `BaroSimHUD`에 표시된다. ⚠ 0.1.7 은 **미커밋/미배포**(HEAD=0.1.6) — 축출 수정을 얹었으므로 배포 전 에디터 종료 후 풀 리빌드·재검증 필요.
